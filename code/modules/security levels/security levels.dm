...
/proc/set_security_level(var/level)
	...
	if(level >= SEC_LEVEL_GREEN && level <= SEC_LEVEL_DELTA && level != security_level)
		switch(level)
			if(SEC_LEVEL_GREEN)
				security_announcement_down.Announce("[config.alert_desc_green]", "Внимание! Уровень безопасности понижен до зеленого")
			...
			if(SEC_LEVEL_DELTA)
				security_announcement_up.Announce("[config.alert_desc_delta]", "ВНИМАНИЕ! КОД ДЕЛЬТА!", new_sound = 'sound/effects/siren.ogg')
