...
/turf/simulated/wall/proc/fail_smash(var/mob/user)
	user << "<span class='danger'>[translation(src, "fail_smash")]</span>"
	take_damage(rand(25,75))

/turf/simulated/wall/proc/success_smash(var/mob/user)
	user << "<span class='danger'>[translation(src, "success_smash")]</span>"
...
/turf/simulated/wall/proc/try_touch(var/mob/user, var/rotting)
	...
	if(rotting)
		if(reinf_material)
			user << "<span class='danger'>[translation(src, "reinf_crumbly")]</span>"
		else
			user << "<span class='danger'>[translation(src, "crumbles")]</span>"
			dismantle_wall()
			return 1
...
/turf/simulated/wall/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if(damage && istype(W, /obj/item/weapon/weldingtool))
		...
		if(WT.remove_fuel(0,user))
			...
			user << "<span class='notice'>[translation(src, "more_fuel")]</span>"
			return
		return
	...
	if(isnull(construction_stage) || !reinf_material)
		...
		if(istype(W,/obj/item/weapon/weldingtool))
			...
			dismantle_verb = translation(P,"drill_verb")
	...
	//Reinforced dismantling.
		...
		switch(construction_stage)
			...
			if(0)
				if(istype(W, /obj/item/weapon/crowbar))
					user << "<span class='notice'>[translation(src, "struggle_sheath")]</span>"
					playsound(src, 'sound/items/Crowbar.ogg', 100, 1)
					if(!do_after(user,100,src) || !istype(src, /turf/simulated/wall) || !user || !W || !T )	return
					if(user.loc == T && user.get_active_hand() == W )
						user << "<span class='notice'>[translation(src, "pry_off_sheat")]</span>"
						dismantle_wall()
					return
